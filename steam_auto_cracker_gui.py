...
try: # Handles Python errors to write them to a log file so they can be reported and fixed more easily.
    ...
    def SettingsButton():
        ...
        tk.Label(fileNamesFrame, text="RetryDelay:").grid(row=0, column=0)
        global RetryDelay_var
        RetryDelay_var = tk.StringVar()
        steamApiEntry = tk.Entry(fileNamesFrame, width=35, textvariable=RetryDelay_var)
        steamApiEntry.grid(row=0, column=1, ipadx=10, ipady=3)
        RetryDelay_var.set(config["Advanced"]["RetryDelay"])
        ttk.Button(fileNamesFrame, text="Save", padding=3, command=lambda: UpdateAdvanced("RetryDelay", RetryDelay_var)).grid(row=0, column=2, ipadx=10)
        
        tk.Label(fileNamesFrame, text="RetryMax:").grid(row=1, column=0)
        global RetryMax_var
        RetryMax_var = tk.StringVar()
        steamApiEntry = tk.Entry(fileNamesFrame, width=35, textvariable=RetryMax_var)
        steamApiEntry.grid(row=1, column=1, ipadx=10, ipady=3)
        RetryMax_var.set(config["Advanced"]["RetryMax"])
        ttk.Button(fileNamesFrame, text="Save", padding=3, command=lambda: UpdateAdvanced("RetryMax", RetryMax_var)).grid(row=1, column=2, ipadx=10)
    ...
    def ResetConfig(resetLevel = 0):
        ...
        if resetLevel == 0 or resetLevel == 2:
            config["Crack"] = {}
            config["Crack"]["SelectedCrack"] = "game_ali213"
