...
try:  # Handles Python errors to write them to a log file so they can be reported and fixed more easily.
    ...
    def OnTkinterError(exc, val, tb):
        ...
        try:
            update_logs(
                "[!!!] A Tkinter Python error occurred! The error has written to error_tkinter.log, please report it on GitHub or cs.rin.ru\nMake sure to blank any personal detail."
            )
        except Exception:
            pass
            ...
            if not req.ok:
                if self.tries < int(config["Advanced"]["RetryMax"]):
                    ...
                    update_logs(
                        ...
                        + config["Advanced"]["RetryDelay"]
                        + "s... ("
                        + str(self.tries)
                        + "/"
                        + config["Advanced"]["RetryMax"]
                        + " tries)"
                    )
                    root.update()
                    ...
                    raise Exception(
                        f"SACRequest: Connection failed after {config['Advanced']['RetryMax']} tries"
                    )
            else:
                self.req = req
    ...
    def update_logs(log_message):
        ...
        logs_text.config(
            state=tk.DISABLED
        )  # Disables modification (prevents the user from writing inside the field)

    def search_game():
        searchGameButton.config(
            state=tk.DISABLED
        )  # Prevents the user from starting multiple searches at the same time
    ...
    def FindInAppList(appName):
        ...
        update_logs(
            "[!] The App was not found, make sure you entered EXACTLY the Steam Game's name (watch it on Steam)"
        )
        update_logs(
            "If you typed it properly, you can try to update the App List. Alternatively, you can try entering the AppID."
        )
        ...
        if option == "1":
            ...
            if "dlc" in data["data"]:
                ...
                if dlcIDsLen >= HIGH_DLC_WARNING:
                    update_logs(
                        f"/!\\ WARNING: This game has more than {HIGH_DLC_WARNING} DLCs. Requests may fail due to Steam rate limiting. If it does, just give it time, it'll eventually manage to retrieve all DLCs."
                    )
            ...
            try:
                req2 = SACRequest(
                    ...
                    + str(appID)
                    + "/random/ajaxgetfilteredrecommendations/?query&count=10000",
                    "RetrieveDLC",
                ).req
            except Exception:
                gameFoundStatus.config(text=f"An error has occurred")
            ...
            if data2["total_count"] == 0:
                ...
                while i + 1 < data2["total_count"]:
                    ...
                    if (
                        dlcID in dlcIDs
                    ):  # data-ds-appid is present 2 times for each AppID currently. This will allow us to not include it if it is already.
                        i -= 1
                        continue
                    ...
                    gameFoundStatus.config(
                        text=f"[2/2] Retrieving DLCs... ({i+1}/{data2['total_count']})"
                    )
                    root.update()

        update_logs(
            f"Finished retrieving all the details about the game {gameName} (appID: {appID})"
        )
    ...
    def CrackGame():
        ...
        if (
            config["Crack"]["SelectedCrack"][:3] == "dlc" and len(dlcIDs) == 0
        ):  # If a dlc only crack has been selected, but the game has no DLC
            update_logs(
        ...
        configDir = os.path.join(
            os.getcwd(), "sac_emu\\" + config["Crack"]["SelectedCrack"]
        )  # "sac_emu/game_ali213" for example
        try:
        ...
        for root_dir, dirs, files in os.walk(folder_path):
            ...
            if (
                config["Preferences"]["Steamless"] == "1"
                ...
                for fileName in files:
                    ...
                    if not os.path.isfile(fileName + ".unpacked.exe"):
                        ...
                        update_logs(
                            "- Couldn't run Steamless on "
                            + fileName
                            + ", it is probably not under DRM."
                        )
            ...
            if "steam_api.dll" in files:
                if config["FileNames"]["SteamAPI"] in files:
                    update_logs(
                        "[!] Seems like a file named "
                        + config["FileNames"]["SteamAPI"]
                        + " is present. This could indicate that steam_api.dll has already been cracked! Overwriting steam_api.dll. No backup of the previous steam_api.dll could be created, and the file has been deleted. "
                        + config["FileNames"]["SteamAPI"]
                        + " has been restored."
                    )
                    os.remove(root_dir + "/steam_api.dll")
                    shutil.move(
                        root_dir + "/" + config["FileNames"]["SteamAPI"],
                        root_dir + "/steam_api.dll",
                    )
                ...
                try:
                    ...
                    update_logs(
                        "[!] steam_api.dll: could not retrieve the file version! Seems like the steam_api.dll file has already been cracked! Aborting..."
                    )
                    EndCrack()
                    return
            ...
            if "steam_api64.dll" in files:
                if config["FileNames"]["SteamAPI64"] in files:
                    update_logs(
                        ...
                        + " is present. This could indicate that steam_api64.dll has already been cracked! Overwriting steam_api64.dll. No backup of the previous steam_api64.dll could be created, and the file has been deleted. "
                        + config["FileNames"]["SteamAPI64"]
                        + " has been restored."
                    )
                    os.remove(root_dir + "/steam_api64.dll")
                    shutil.move(
                        root_dir + "/" + config["FileNames"]["SteamAPI64"],
                        root_dir + "/steam_api64.dll",
                    )
                ...
                try:
                    ...
                    update_logs(
                        "[!] steam_api64.dll: could not retrieve the file version! Seems like the steam_api64.dll file has already been cracked! Aborting..."
                    )
                    EndCrack()
                    return
                    ...
                    if os.path.isfile(
                        ...
                        if fileName == "steam_api.dll" or fileName == "steam_api64.dll":
                            if (
                                ...
                                update_logs(
                                    ...
                                    + relativeRootDir
                                    + fileName
                                    + " because of the set crack approach"
                                )
                                continue
                        ...
                        if (
                            ...
                            update_logs(
                                ...
                                + relativeRootDir
                                + fileName
                                + " file because no backup file name is set"
                            )
                        elif os.path.isfile(
                            os.path.join(dllAbsoluteRelativeLocation, newName)
                        ):  # A backup of this file already exists, the game might already be cracked, abort!
                            update_logs(
                                "[!] Seems like the backup of "
                                + relativeRootDir
                                + fileName
                                + " file already exists! This could indicate that the game has already been cracked. Overwriting it. No backup of "
                                + relativeRootDir
                                + fileName
                        ...
                        fileName == "steam_api.dll" or fileName == "steam_api64.dll"
                    ):  # No existing file, and this file is the steam_api(64).dll one
        ...
        if not cracked:
            ...
            update_logs(
                "The game has been cracked successfully! (If you attempt to crack if again, SAC will try its best to make it work, but will let some leftovers of old cracks.)"
            )
    ...
    def SettingsButton():
        ...
        def on_mousewheel(event):
            scrollCanvas.yview_scroll(
                int(-1 * (event.delta / 120)), "units"
            )  # Some magic I guess. Huge thanks to LLM who probably stole this code from someone.
        ...
        ttk.Label(scrollFrame, text="Updates:", font=FONT3, padding=0).pack(
            padx=(6, 0), pady=(10, 0), anchor="w"
        )
        ttk.Label(
            scrollFrame,
            text="This will search the latest version on GitHub.\nIf you're afraid of leaking your IP to GitHub, use a VPN and/or disable auto updating.",
            font=FONT4,
            padding=0,
            foreground="#575757",
            wraplength=600,
        ...
        ttk.Label(scrollFrame, text="Crack approach:", font=FONT3, padding=0).pack(
            padx=(6, 0), pady=(10, 0), anchor="w"
        )
        ...
        # Steamless (Steamless)
        ttk.Label(scrollFrame, text="Steamless:", font=FONT3, padding=0).pack(
            padx=(6, 0), pady=(10, 0), anchor="w"
        )
        ttk.Label(
            scrollFrame,
            text="This will allow SAC to bypass the SteamStub DRM if it is used.",
            font=FONT4,
            padding=0,
            foreground="#575757",
            wraplength=600,
        ...
        # FileNames
        ttk.Label(scrollFrame, text="File names:", font=FONT3, padding=0).pack(
            padx=(6, 0), pady=(10, 0), anchor="w"
        )
        ttk.Label(
            scrollFrame,
            text="You can enter the name the different files will have.\nIt is recommended to keep the default ones.",
            font=FONT4,
            padding=0,
            foreground="#575757",
            wraplength=600,
        ...
        ttk.Button(
            ...
            text="Save",
            padding=3,
            command=lambda: UpdateFileName("SteamAPI64", SteamApi64_var),
        ).grid(row=1, column=2, ipadx=10)
        ...
        ttk.Button(
            ...
            text="Save",
            padding=3,
            command=lambda: UpdateFileName("BakSuffix", BakSuffix_var),
        ).grid(row=3, column=2, ipadx=10)

        # Advanced
        ttk.Label(scrollFrame, text="Advanced:", font=FONT3, padding=0).pack(
            padx=(6, 0), pady=(10, 0), anchor="w"
        )
        ttk.Label(
            scrollFrame,
            text="Advanced settings, don't modify unless you know what you're doing.",
            font=FONT4,
            padding=0,
            foreground="#575757",
            wraplength=600,
        ...
        ttk.Button(
            ...
            text="Save",
            padding=3,
            command=lambda: UpdateAdvanced("RetryDelay", RetryDelay_var),
        ).grid(row=0, column=2, ipadx=10)
        ...
        ttk.Button(
            ...
            text="Save",
            padding=3,
            command=lambda: UpdateAdvanced("RetryMax", RetryMax_var),
        ).grid(row=1, column=2, ipadx=10)
        ...
        advBypassGameVerification = ttk.Checkbutton(
            scrollFrame,
    ...
    def DisplayCrackList():
        ...
        for k, v in crackList.items():
            ttk.Radiobutton(
                settings_frame1,
                text=v[0],
                variable=SelectedCrack_var,
                value=k,
                command=lambda: UpdateSelectedCrack(),
            ).grid(row=rowNum, column=0, sticky="w")
            rowNum += 1
            if len(v) > 1:  # Contains a description
                tk.Label(
                    ...
                    wraplength=700,
                    justify="left",
                ).grid(row=rowNum, column=0, sticky="w", ipadx=20)
                rowNum += 1
    ...
    def UpdateSelectedCrackDisplay():
        selectCrackButton.config(
            text=crackList[config["Crack"]["SelectedCrack"]][0]
        )  # Display the name of the selected crack on the select crack button in the root window
    ...
    def DisplayUpdate():
        ...
        updateDisplayButtonUpdate = ttk.Button(
            updateDisplayButtonsFrame, text="Update now", command=UpdateSAC, padding=3
        )
        updateDisplayButtonUpdate.grid(row=0, column=0)

        global updateDisplayButtonCopy
        updateDisplayButtonCopy = ttk.Button(
            updateDisplayButtonsFrame,
            text="Copy the release URL",
            command=CopyReleaseURL,
            padding=3,
        )
        updateDisplayButtonCopy.grid(row=0, column=1, padx=(50, 0))

        global updateDisplayButtonClose
        updateDisplayButtonClose = ttk.Button(
            updateDisplayButtonsFrame,
            text="Don't update yet",
            command=top.destroy,
            padding=3,
        )
    ...
    def UpdateSAC():
        ...
        if os.path.isfile("steam_auto_cracker_gui_autoupdater.exe"):
            try:
                ...
                updateDisplayStatusLabel.config(
                    text="An error occurred. The autoupdater (steam_auto_cracker_gui_autoupdater.exe) already exists.\nSAC couldn't delete the autoupdater. Please try to remove it yourself, or try again."
                )
                root.update()
                return
    ...
    updatesButton = ttk.Button(
        updatesFrame, text="Check for updates", command=CheckUpdates, padding=0
    )
    updatesButton.grid(row=0, column=0)
    updatesFrame.pack(pady=(0, 20))

    ttk.Button(root, text="Settings", command=SettingsButton, padding=8).pack(
        pady=(0, 20), anchor="center"
    )
    ...
    selectedFolderFrame = tk.Frame(
        root
    )  # This frame will contain the label. This is so we can resize the root window properly when the text is empty.
    selectedFolderFrame.pack()
    tk.Frame(
        selectedFolderFrame, width=1, height=1
    ).pack()  # 1x1 frame, else selectedFolderFrame will not update its size after it is emptied (by selectedFolderLabel.pack_forget)
    ...
    tk.Frame(
        frameGame, width=1, height=1
    ).pack()  # 1x1 frame, else frameGame will not update its size after it is emptied (by frameGame2.pack_forget)
    frameGame2 = ttk.Frame(
        frameGame
    ...
    updateAppListButton = ttk.Button(
        frame4, text="Update the App List", padding=0, command=UpdateAppList
    )
    ...
    selectCrackButton = ttk.Button(
        selectedCrackFrame, text="None", padding=5, command=DisplayCrackList
    )
